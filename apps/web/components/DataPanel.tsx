...
import { 
  Download, Trash2, User, MessageSquare, FileText, Sigma, Grip,
  MapPin, Building, Mail, Database, Ban, Check, Hash, Euro, AlertCircle
} from 'lucide-react';
    ...
    <div className="flex flex-col h-full bg-white text-black">
      ...
      <div className="flex-shrink-0 p-6 border-b bg-white">
        <div className="flex items-center justify-between">
          ...
          <Badge variant="outline" className="text-blue-600 border-blue-600 bg-blue-50 font-semibold py-1 px-3">Needs Review</Badge>
        </div>
        <div className="flex items-center gap-2 mt-4">
          ...
          <Button variant="outline" size="icon" className="ml-auto text-red-600 border-red-300 hover:bg-red-50 hover:text-red-700 rounded-lg" title="Reject"><Ban className="h-5 w-5" /></Button>
          <Button className="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded-lg" onClick={handleSave} disabled={isSaving}>
            {isSaving ? (invoiceId ? 'Updating...' : 'Saving...') : <><Check className="h-5 w-5 mr-2" /> {invoiceId ? 'Update' : 'Approve'}</>}
          </Button>
        </div>
      </div>
      ...
      <div className="flex-grow overflow-auto p-6 bg-white">
        <Accordion type="multiple" defaultValue={['customer-info', 'invoice-details', 'summary', 'line-items']} className="w-full">
          ...
          <AccordionItem value="customer-info" className="border-b">
            ...
            <AccordionContent className="p-6 pt-4">
                <div className="grid grid-cols-[auto,1fr] items-center gap-x-4 gap-y-4">
                    <div className="flex items-center gap-2 text-sm text-gray-600 font-medium"><User className="h-4 w-4"/> Customer Name</div>
                    ...
                    <div className="flex items-center gap-2 text-sm text-gray-600 font-medium"><MapPin className="h-4 w-4"/> Address</div>
                    ...
                    <div className="flex items-center gap-2 text-sm text-gray-600 font-medium"><Building className="h-4 w-4"/> Vendor Address</div>
                    ...
                    <div className="flex items-center gap-2 text-sm text-gray-600 font-medium"><Mail className="h-4 w-4"/> Postal Code</div>
                    <Input placeholder="478392" value={formData.vendor.taxId || ''} onChange={(e) => handleInputChange('vendor', 'taxId', e.target.value)} />
                    <div className="flex items-center gap-2 text-sm text-gray-600 font-medium"><Database className="h-4 w-4"/> Customer Code<span className="text-red-500">*</span></div>
                    <Select><SelectTrigger className="border-red-300 text-red-500"><SelectValue placeholder="Add Code" /></SelectTrigger><SelectContent><SelectItem value="code1">Code 1</SelectItem></SelectContent></Select>
                </div>
            </AccordionContent>
          </AccordionItem>
          ...
          <AccordionItem value="invoice-details" className="border-b">
            ...
            <AccordionContent className="p-6 pt-4">
                <div className="grid grid-cols-[auto,1fr] items-center gap-x-4 gap-y-4">
                    <div className="flex items-center gap-2 text-sm text-gray-600 font-medium"><Hash className="h-4 w-4"/> PO Number</div>
                    <Input placeholder="90038195" value={formData.invoice.poNumber || ''} onChange={(e) => handleInputChange('invoice', 'poNumber', e.target.value)} />
                    <div className="flex items-center gap-2 text-sm text-gray-600 font-medium"><Hash className="h-4 w-4"/> PO Date</div>
          ...
          <AccordionItem value="summary" className="border-b">
            ...
            <AccordionContent className="p-6 pt-4">
                <div className="grid grid-cols-[auto,1fr] items-center gap-x-4 gap-y-4">
                    <div className="flex items-center gap-2 text-sm text-gray-600 font-medium">Document Type</div>
                    <Input placeholder="Rechnung" value={formData.invoice.number || ''} onChange={(e) => handleInputChange('invoice', 'number', e.target.value)} />
                    <div className="flex items-center gap-2 text-sm text-gray-600 font-medium"><Euro className="h-4 w-4"/> Currency Short Form</div>
                    <Input placeholder="€ EUR - Euro" value={formData.invoice.currency || ''} onChange={(e) => handleInputChange('invoice', 'currency', e.target.value)}/>
                    <div className="flex items-center gap-2 text-sm text-red-500 font-medium"><AlertCircle className="h-4 w-4"/>Sub Total</div>
          ...
          <AccordionItem value="line-items" className="border-none">
            ...
            <AccordionContent className="p-4">
                <Table>
                  <TableHeader><TableRow><TableHead>Description</TableHead><TableHead className="w-24">Qty</TableHead><TableHead className="w-28">Unit Price</TableHead><TableHead className="w-28">Total</TableHead><TableHead className="w-12"></TableHead></TableRow></TableHeader>
                  <TableBody>
